...
typedef enum
{
    ...
    /*!< @brief Process sink element */
    TYPE_ELEMENT_PROCESS_SINK = BUILD_ELEMENT(0, 1, 7),
...
typedef enum
{
    AUDIOSRC_DEFAULT,                /*!< @brief Audio source device using default interface */
...
typedef enum
{
    AUDIO_SINK_DEVICE_TYPE_DEFAULT,    /*!< @brief Audio sink device using default interface */
...
typedef struct
{
    ...
    osa_msgq_handle_t mutex;
    osa_msgq_handle_t cond;
    volatile bool *processed;                     /*!< @brief Flag to indicate streamer msg processed */
...
typedef struct
{
    OSA_MSGQ_HANDLE_DEFINE(mq, MSG_POOL_NUM_MSGS, STREAMER_MSG_SIZE);
    /*!< @brief Message Queue for sending message out of streamer task */
    volatile bool is_mq_created;
    /*!< @brief is message queue "mq" created */
} STREAMER_OUT_MESSAGE_CHANNEL_T;
...
typedef struct
{
    ...
    OSA_MSGQ_HANDLE_DEFINE(streamer_mq, MSG_POOL_NUM_MSGS, STREAMER_MSG_SIZE);
    /*!< @brief Message queue for receive message */
    STREAMER_OUT_MESSAGE_CHANNEL_T message_channel_out;
    /*!< @brief communication channel for sending messages from streamer task to the application */
