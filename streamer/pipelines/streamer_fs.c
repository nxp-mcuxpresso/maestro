                               ...
                               STREAMER_T *task_data)
{
    ...
    ret = create_element(&task_data->elems[pipeline_index][ELEMENT_SRC_INDEX], TYPE_ELEMENT_AUDIO_PROC, 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "create element(%d) failed:%d\n", ELEMENT_SRC_INDEX, ret);
        goto err_catch;
    }
#endif
    /* CREATE AUDIO SINK */
    ...
    ret = add_element_pipeline(task_data->pipes[pipeline_index], task_data->elems[pipeline_index][ELEMENT_SRC_INDEX],
                               level++);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "add element(%d) failed:%d\n", ELEMENT_SRC_INDEX, ret);
    ...
    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_DECODER_INDEX], 0,
                        task_data->elems[pipeline_index][ELEMENT_SRC_INDEX], 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "link element(%d) to element(%d) failed:%d\n",
                         ELEMENT_DECODER_INDEX, ELEMENT_SRC_INDEX, ret);
        goto err_catch;
    }

    /* Link the SSRC_PROC to the audio sink */
    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_SRC_INDEX], 0,
    ...
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "link element(%d) to element(%d) failed:%d\n",
                         ELEMENT_DECODER_INDEX, ELEMENT_SPEAKER_INDEX, ret);
        goto err_catch;
    }
...
int streamer_destroy_fs_pipeline(int8_t pipeline_index, STREAMER_T *task_data)
{
    ...
    ret = unlink_elements(task_data->elems[pipeline_index][ELEMENT_SRC_INDEX], 0,
                          task_data->elems[pipeline_index][ELEMENT_SPEAKER_INDEX], 0);
#else
    ret = unlink_elements(task_data->elems[pipeline_index][ELEMENT_DECODER_INDEX], 0,
                          task_data->elems[pipeline_index][ELEMENT_SPEAKER_INDEX], 0);
...
#if (SSRC_PROC == 1)
    ret =
        remove_element_pipeline(task_data->pipes[pipeline_index], task_data->elems[pipeline_index][ELEMENT_SRC_INDEX]);
    if (STREAM_OK != ret)
    {
...
#if (SSRC_PROC == 1)
    ret = destroy_element(task_data->elems[pipeline_index][ELEMENT_SRC_INDEX]);
    if (STREAM_OK != ret)
    {
    ...
    task_data->elems[pipeline_index][ELEMENT_SRC_INDEX] = (uintptr_t)NULL;
#endif
