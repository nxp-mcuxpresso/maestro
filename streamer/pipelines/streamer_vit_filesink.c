                                         ...
                                         STREAMER_T *task_data)
{
    ...
    ret = create_element(&task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], TYPE_ELEMENT_AUDIO_SRC, 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "create element(%d) failed:%d\n", ELEMENT_MICROPHONE_INDEX, ret);
        goto err_catch;
    }

#ifdef VOICE_SEEKER_PROC
    ret = create_element(&task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], TYPE_ELEMENT_AUDIO_PROC, 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "create element(%d) failed:%d\n", ELEMENT_VOICESEEKER_INDEX, ret);
        goto err_catch;
    }
#endif

    ret = create_element(&task_data->elems[pipeline_index][ELEMENT_VIT_INDEX], TYPE_ELEMENT_AUDIO_PROC, 0);
    if (STREAM_OK != ret)
    {
    ...
    ret = add_element_pipeline(task_data->pipes[pipeline_index],
                               task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], level++);
    if (STREAM_OK != ret)
    {
...
#ifdef VOICE_SEEKER_PROC
    ret = add_element_pipeline(task_data->pipes[pipeline_index],
                               task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], level++);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "add element(%d) failed:%d\n", ELEMENT_VOICESEEKER_INDEX, ret);
        goto err_catch;
...
#ifdef VOICE_SEEKER_PROC
    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], 0,
                        task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "link element(%d) to element(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, ELEMENT_VOICESEEKER_INDEX, ret);
        goto err_catch;
    }

    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], 0,
                        task_data->elems[pipeline_index][ELEMENT_VIT_INDEX], 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "link element(%d) to element(%d) failed:%d\n",
                         ELEMENT_VOICESEEKER_INDEX, ELEMENT_VIT_INDEX, ret);
        goto err_catch;
    }
#else
    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], 0,
    ...
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "link element(%d) to element(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, ELEMENT_VIT_INDEX, ret);
        goto err_catch;
    }
#endif

    ret = link_elements(task_data->elems[pipeline_index][ELEMENT_VIT_INDEX], 0,
    ...
    prop.prop                      = PROP_MICROPHONE_SET_DEVICE_NAME;
    prop.val                       = (uintptr_t)&audsrc_name;

    ret = element_set_property(task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], prop.prop, prop.val);

    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "set element(%d) property(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, PROP_MICROPHONE_SET_DEVICE_NAME, ret);
        goto err_catch;
    }
    ...
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "set element(%d) property(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, PROP_MICROPHONE_SET_SAMPLE_RATE, ret);
        goto err_catch;
    }
    ...
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "set element(%d) property(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, PROP_MICROPHONE_SET_DUMMY_TX_ENABLE, ret);
        goto err_catch;
    }
    ...
    prop.prop = PROP_MICROPHONE_SET_DEVICE_TYPE;
    ...
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "set element(%d) property(%d) failed:%d\n",
                         ELEMENT_MICROPHONE_INDEX, PROP_MICROPHONE_SET_DEVICE_TYPE, ret);
        goto err_catch;
    }

    /* Set the file sink output device
    ...
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "set element(%d) property(%d) failed:%d in raw mode\n",
                         ELEMENT_FILE_SINK_INDEX, PROP_FILESINK_RAW_WRITE_MODE, ret);
        goto err_catch;
    }
...
err_catch:
    ...
    */
    if (STREAM_OK != ret)
    {
        streamer_destroy_vit_filesink_pipeline(pipeline_index, task_data);
    }
...
int streamer_destroy_vit_filesink_pipeline(int8_t pipeline_index, STREAMER_T *task_data)
{
...
#ifdef VOICE_SEEKER_PROC
    ret = unlink_elements(task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX], 0,
                          task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], 0);
    if (STREAM_OK != ret)
    {
    ...
    ret = unlink_elements(task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX], 0,
                          task_data->elems[pipeline_index][ELEMENT_VIT_INDEX], 0);
    if (STREAM_OK != ret)
    {
    ...
    ret = unlink_elements(task_data->elems[pipeline_index][ELEMENT_VIT_INDEX], 0,
                          task_data->elems[pipeline_index][ELEMENT_FILE_SINK_INDEX], 0);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "Failed to unlink file sink and vit\n");
        return ret;
    }

    /* remove_elements from pipeline*/
    ret = remove_element_pipeline(task_data->pipes[pipeline_index],
                                  task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX]);
    if (STREAM_OK != ret)
    {
...
#ifdef VOICE_SEEKER_PROC
    ret = remove_element_pipeline(task_data->pipes[pipeline_index],
                                  task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX]);
    if (STREAM_OK != ret)
    {
    ...
    /* destroy elements*/
    ret = destroy_element(task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX]);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "Failed to remove audio src\n");
        return ret;
    }
    task_data->elems[pipeline_index][ELEMENT_MICROPHONE_INDEX] = (uintptr_t)NULL;

#ifdef VOICE_SEEKER_PROC
    ret = destroy_element(task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX]);
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "Failed to remove voiceseeker proc\n");
        return ret;
    }
    task_data->elems[pipeline_index][ELEMENT_VOICESEEKER_INDEX] = (uintptr_t)NULL;
#endif
    ...
    if (STREAM_OK != ret)
    {
        STREAMER_LOG_ERR(DBG_CORE, ERRCODE_INTERNAL, "Failed to remove file sink\n");
        return ret;
    }
