...
osa_status_t OSA_TaskCreate(osa_task_handle_t taskHandle, const osa_task_def_t *thread_def, osa_task_param_t task_param)
{
    ...
    /* Create a task */
    z_taskHandle->handle =
        k_thread_create(&z_taskHandle->controlBlock, (k_thread_stack_t *)z_taskHandle->stackPtr, thread_def->stacksize,
                        s_OSA_TaskCallback, (void *)thread_def->pthread, (void *)task_param,
...
osa_task_handle_t OSA_TaskGetCurrentHandle(void)
{
    ...
    {
        OSA_ASSERT(false,
                   "First initialize OSA by ccalling OSA_Init bbefore any task creation to be able to use this "
                   "function correctly");
        return NULL;
    }
                           ...
                           osa_event_flags_t *pSetFlags)
{
    ...
    if (waitAll)
    {
        setFlags = k_event_wait_all(&z_eventHandle->handle, flagsToWait, false, s_OSA_ConvertTimeoutToZephyr(millisec));
    }
    else
    {
